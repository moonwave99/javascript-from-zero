...
The `printUserInfo` function does not care where the passed objecs are stored. It just wants to deal with an object that has `firstName`, `lastName`, `age`, `email` and `address` properties.
