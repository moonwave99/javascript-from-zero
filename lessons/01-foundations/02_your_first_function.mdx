...
In this lesson we'll introduce one of **the most important concepts in programming**, if not the most important at all! You won't be able to perceive such importance yet, but since it's also one of the hardest to digest, the sooner you familiarise with it, the better.
...
Spaces in the name are not allowed though, that's why if a function name contains multiple words, we capitalise those after the first. This convention is called [camel case][mdn:camel-case] be cause of the similarity of the capital letters to the humps of a camel's back 🐪.
...
<details>
  <summary>**Bonus:** the code above can be written using just two istances of `console.log`, can you guess how?</summary>
...
We haven't mentioned it explicitely yet, but you can call functions from inside functions! That will be the rule pretty soon. We'll get there together, don't fret!
</details>
:::
