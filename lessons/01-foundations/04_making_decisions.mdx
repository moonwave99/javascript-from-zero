...
Wait wait, what are those `true` and `false` things now? They are called **boolean valuse**, we'll cover them in detail soon. For now think that they are **answers to a yes or no question**.
...
- it has three possible behaviours: one if the weather is sunny, one if it's rainy, a further one otherwise;
...
The comparison is responsibility of the **comparison operator** (`==`), depicted with two (!) equals. Why two equals and not one? Because the single equals is another operator, whose purpose is going to be discussed in two lessons from now.
...
Dealing with logic in programming can be convoluted at first. We'll see a lot of examples so you can familiarise with the syntax. For now focus as much as possible on the idea that **different inputs** lead to **different outputs** depending on how they match the internal function decisions.
...
But before you get carried away, we'll see how this is not particularly good code. We want our function to be as specialised as possible, without the need of taking too many decisions. We'll discuss the strategies to avoid the common pitfalls.
