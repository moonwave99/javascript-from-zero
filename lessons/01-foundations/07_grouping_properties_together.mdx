...
`true` and `false` are called **boolean values**. They describe something that can have just two possible states. A user is either verified or not. The sun is either shining or it is hiding behing clouds. A person can be of major age or not in a specific jurisdiciton. An integer number is either even or odd.

We feeded the `isVerified` property of the `user` object as the condition of an **if statement**. Boolean values and conditional are really meant to work together!

See also that there is no `else` block following the `if` block. If we don't want to log anything in case the user is not verified, we don't have to provide an alternative behaviour of our program.
...
See how now just the `else` branch is excecuted, since `user.isVerified` is false.
