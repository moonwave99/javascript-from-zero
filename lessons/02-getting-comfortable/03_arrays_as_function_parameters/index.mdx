...
Great! Let's see in detail how we reached this result. In order to concentrate on what's going on inside the loop let's ignore the return statement and the final log. Ah, from now on we'll use `i++` instead of `i = i+1`, I hope you don't mind.
