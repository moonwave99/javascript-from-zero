...
In the first module of the course we have dealt mostly with single entities, like greeting a single user or calculating the discounted price of an item. Very often we'll have to deal with homogenous collection of entities, e.g.:
...
The structure we have just seen is a `for` statement. It consist of **three expressions** separated by semicolons and enclosed in parentheses, followed by a **code block** in curly brackets:
...
- **the initialization**: they allow to declare one or more variables;
- **the condition**: if evaluates to `true`, the code block is executed - if not, the program evaluation continues with what follows the for statement;
- **the afterthought**: to be evaluated after the code block.
...
- condition evaluated 3 is **not** lesser than three!
- we move on with the program.

It looks like a lot of considerations, because they are. But you have to appreciate the transparency of the language. It's crucial that you spend time doing exactly these petulant observations now that the code is just logging a variable value, before the code gets entangled.
...
You absolutely don't have to start from 0:
...
More commonly you'll see the `i = i+1` written as `i++`. The double plus symbol is called the **increment operator** and updates the operand to its current value plus one:
